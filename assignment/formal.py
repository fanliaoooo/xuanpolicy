...
inputs = input_data.reshape(input_data.shape[0],6,-1)
#预处理数据
...
input_valid = input_valid.reshape(input_valid.shape[0],6,-1)
...
input_valid = input_valid.reshape(input_valid.shape[0],-1)
...
input_test = input_test.reshape(input_test.shape[0],6,-1)
    ...
    if mode == 0: #0为验证数据
        ...
        with torch.no_grad():
            for i in range(test_input.size(0)):
                ...
                running_loss += loss.item()
                outputs = outputs.reshape(outputs.shape[0],6,6)
                _,predicted = torch.max(outputs,1)
                label = label.reshape(6,6)
                _,label = torch.max(label,1)
                total += 6
                correct += (predicted == label).sum().item()

        accuracy = correct / total
        print(f'Loss_on_valid_set: {running_loss/400:.4f}')
...
model.to('cuda:0')
# 定义损失函数和优化器
criterion = nn.BCEWithLogitsLoss(reduction='mean')  # 交叉熵损失函数，用于多分类问题
...
#训练
train(model,input_data, label, criterion, optimizer,scheduler,500,input_data.size(0),16)
